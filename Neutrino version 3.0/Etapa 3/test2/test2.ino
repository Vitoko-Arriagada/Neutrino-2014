...
int max_speed=150; //90 es perfect
int vel_giro=150;
...
void estabilizar(){
	...
	while(1){
		...
		error_value = int(proportional * 0.1 + ((integral * Ki)/100) + derivative * 0.1);
		if (error_value< -255){
			error_value = -255;
		}
		if (error_value> 255){
		...
		if (error_value < 0){
			right_speed = -error_value;
			left_speed = error_value;
		}else{
			right_speed = -error_value;
			left_speed = error_value;
		}
...
void sigue_lineas(){
	...
	while(1){
		...
		motores(right_speed, left_speed);
                if(sensors[1] > 700 && sensors[3] > 700 && sensors[5] > 700){
			return;
		}
		if(sensors[0] < 350 || sensors[6] < 350){
			return;
		}
...
void loop(){
      ...
      */
      ...
      while(1){
		...
		int found_right=0;
		leer_sensores();
		if(sensors[0] < 700){found_left=1;}
		if(sensors[6] < 700){found_right=1;}
		motores(80,80);
		delay(40);
		leer_sensores();
		if(sensors[1] < 700 || sensors[3] < 700 || sensors[5] < 700){found_straight = 1;}
		if(sensors[0] < 900 && sensors[2] < 900 && sensors[6] < 900){break;}
